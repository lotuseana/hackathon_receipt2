...
function App() {
  ...
  const getSmartTip = async (allEntries, newEntryOrEntries) => {
    ...
    if (Array.isArray(newEntryOrEntries)) {
      prompt = `You are Budgie, a cute, smart, friendly bird mascot for a budgeting app. Give concise, friendly, and actionable budgeting tips in a warm but succinct tone. Use at most one emoji per tip. Given the user's full spending history (as an array of entries) and the most recent receipt upload (as an array of new items), provide a positive budgeting tip or insight. Focus on helping the user spend smarter, spot trends, or stay motivated. Categories are not editable. Do not repeat previous tips. Return only the tip, no extra text or formatting.\n\nAll Entries: ${JSON.stringify(allEntries)}\nNew Receipt Items: ${JSON.stringify(newEntryOrEntries)}`;
    } else {
